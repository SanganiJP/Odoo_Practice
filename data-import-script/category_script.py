...
def import_product_categories():
    ...
    for record in sheet.iter_rows(min_row=2, values_only=True):
        category_path = record[0]
        if not category_path:
            continue

        parent_id = False
        for name in category_path.split(' / '):
            existing = odoo.env['product.category'].search([('name', '=', name), ('parent_id', '=', parent_id)], limit=1)
            if existing:
                parent_id = existing[0]
            else:
                parent_id = odoo.env['product.category'].create({
                    'name': name,
                    'parent_id': parent_id
                })
